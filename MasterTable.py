...
def extract_positive_time_unit(sample):
    ...
    match = re.search(r'([dwm])(\d+)', sample)  # Matches 'd', 'w', 'm' followed by digits
    if match:
        unit = match.group(1)
        ...
        if unit == 'd':
            ...
            return f"{value * 7} days"  # Assuming 1 week = 7 days
        elif unit == 'm':
            return f"{value * 30} days"  # Assuming 1 month = 30 days
    return None

def extract_negative_time_unit(sample):
    """Extracts negative time units from the sample string."""
    match = re.search(r'([dwm])-?(\d+)', sample)  # Matches 'd', 'w', 'm' followed by '-' and digits
    if match:
        unit = match.group(1)
        value = int(match.group(2))
...
def combine_and_organize(input_directory, output_directory, processed_files=None):
    """
    Combine multiple organized data files into separate master tables for each sample group.

    Args:
        input_directory (str): Directory where organized data files are located.
        output_directory (str): Directory where the master tables will be saved.
    ...
    try:
        ...
        # Initialize dictionaries to store DataFrames for each sample group
        master_dfs = {
            '101': pd.DataFrame(),
            '201': pd.DataFrame(),
            '301': pd.DataFrame()
        ...
        # Iterate through each organized data file and append to corresponding master_df
        for file in organized_files:
            ...
            # Process each sheet (assay) and append to corresponding master_df
            for sheet_name, sheet_df in df.items():
                if 'Sample' in sheet_df.columns:
                    # Add new column for days based on the Sample column
                    sheet_df['Days'] = sheet_df['Sample'].apply(extract_time_unit)

                    # Determine the sample group and add to the corresponding master_df
                    sample_group = None
                    ...
                    if sample_group:
                        ...
                        # Append to corresponding master_df
